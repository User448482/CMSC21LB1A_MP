...
void debug(string text) {
    ofstream outLog;
    outLog.open("log.txt", ios::app); // ios:app learned from: https://www.cplusplus.com/doc/tutorial/files/
    outLog << text.append("\n");
    outLog.close();
}

void cls() { // Clears screen | Learned from: https://stackoverflow.com/questions/17335816/clear-screen-using-c
    //cout << "\033[2J\033[1;1H"; // before prof's suggestion
    system("cls||clear"); // prof's suggestion + found on: https://stackoverflow.com/questions/228617/how-do-i-clear-the-console-in-both-windows-and-linux-using-c
}
...
void doTheThing() {
    ...
    while (toNextRound == true) {
        ...
        */
        ...
        debug("nBombs: " + to_string(nBombs));
        nMoves = randomInt(0, 5);

        debug("Room before generate bills:");
        s = "";
        for (vector<cell> v: room) {
            for (cell c: v) {
                s.append(to_string(c.value));
            }
        }
        s.append("\n");
        debug(s);
        ...
        debug("Room after generate bills:");
        s = "";
        for (vector<cell> v: room) {
            for (cell c: v) {
                s.append(to_string(c.value));
        ...
        s = "Before shuffling, bombsCoords: ";
        for (int i: bombsCoords) {
        ...
        for (int i=0; i<nBombs; i++) {
            ...
            if (tempInt < 10) { // special case, 0-9 should be 00-09
                ...
                debug("Bomb placed at: (0," + to_string(tempInt%10) + ')');
                unsafeRows.insert(0);
                unsafeColumns.insert(tempInt%10);
            } else {
                room[tempInt/10][tempInt%10].value = 0;
                debug("Bomb placed at: (" + to_string(tempInt/10) + ',' + to_string(tempInt%10) + ')');
        ...
        // CHECK ANSWER
        if (correctAnswer == answer || answer == 42) { // debugging: change this to != if you want the answer to always be correct | 42 is a cheat number
        ...
        // INCREASE ROOM SIZE FOR NEXT ROUND
        if (nRows < 15 && nColumns < 15) {
            if (nRows <= nColumns) {
                nRows++;
            } else {
                nColumns++;
